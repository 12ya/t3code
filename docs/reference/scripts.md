...
- Dev commands default `V12CODE_HOME` to `~/.v12code` — the same shared home the desktop/production app uses. Override with `--home-dir` (see below) to keep dev state separate.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `vp run dev -- --home-dir ~/.v12code-2`
...
- Example: `V12CODE_DEV_INSTANCE=branch-a vp run dev:desktop`
